...
def dilated_conv2d(inputs, filters, kernel_size, dilation_rate=1, strides=1, padding='SAME'):
    """
    实现空洞卷积的函数。

    参数:
    inputs: 输入的Tensor，形状为 [batch_size, height, width, channels]。
    filters: 卷积核的数量 (输出通道数)。
    kernel_size: 卷积核的大小 (int 或者 tuple)。
    dilation_rate: 空洞卷积的扩展率，默认为1。
    strides: 步长 (int 或者 tuple)，默认为1。
    padding: 填充方式，"SAME" 或者 "VALID"，默认为"SAME"。
    ...
    conv_filter = tf.Variable(
        tf.random.normal([kernel_size[0], kernel_size[1], inputs.shape[-1], filters]),
        dtype=tf.float32
    )
